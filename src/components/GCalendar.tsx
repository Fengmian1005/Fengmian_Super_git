...
export default class GCalendar extends React.Component {
  ...
  state = {
    ...
    jobWorkerNumbers: 1,
    jobHoursPerDay: 0,
    openSettingModal: localStorage.getItem('isAuthorized') == '0' ? true : false,
  }
  ...
  onBeforeUpdateSchedule = async (event: any) => {
    ...
    // const ed = event.changes.end.toDate()
    sd.setHours(12)
    ...
    const strSD = [year, month, day].join('-')
    ...
    if (month.length < 2) month = '0' + month
    if (day.length < 2) day = '0' + day
    const strED = [year, month, day].join('-')
  ...
  onBeforeCreateSchedule = (event: any) => {
    ...
    //const Difference_In_Time = ed.getTime() - sd.getTime()
    //const Difference_In_Days = Math.ceil(Difference_In_Time / (1000 * 3600 * 24))
    //const jobHours = Math.round(8 * Difference_In_Days)
    const jobTitle = ''
    ...
    const jobHours = Math.round(8 * job_days)
    this.setState({
      openModal: true,
      jobStartDate: sd,
      jobEndDate: ed,
  ...
  handleModalSave = async () => {
    ...
    if (this.state.isNewJob) {
      ...
      if (cref != null) {
        ...
        const colorList = ['#1abc9c','#2ecc71', '#16a085', '#27ae60', '#2980b9', '#f1c40f', '#e67e22', '#e74c3c']
        const bgc = colorList[Math.floor(Math.random() * 8)]
        const new_sc = {
          id: newID,
          title:
            this.state.jobTitle +
        ...
        <div style={{ width: '90%' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <Form.Control as="select" size="sm" onChange={this.handleChangeView}>
                <option> Month </option>
                  ...
                  <Row>
                    ...
                    <Col sm={6} style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <Form.Control
                        ...
                        defaultValue={this.state.jobHoursPerDay}
                        min="0"
